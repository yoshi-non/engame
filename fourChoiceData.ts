const fourChoiceData = [
    {
        id: 1,
        question: "正解のスペルはどれ？",
        choiceA: "Github",
        choiceB: "GitHub",
        choiceC: "Git-hub",
        choiceD: "Git-Hub",
        answer: "GitHub",
    },
    {
        id: 2,
        question: "正解のスペルはどれ？",
        choiceA: "Read Me",
        choiceB: "READ Me",
        choiceC: "readme",
        choiceD: "README",
        answer: "README",
    },
    {
        id: 3,
        question: "Gitのmasterブランチがmainブランチに切り替わった理由はどれ？",
        choiceA: "バージョンをアップデートしたため",
        choiceB: "差別用語だったため",
        choiceC: "管理するのが難しくなったため",
        choiceD: "意味が伝わりにくかったため",
        answer: "差別用語だったため",
    },
    {
        id: 4,
        question: "Rubyの名前の由来はどれ？",
        choiceA: "宝石のように輝く言語になってほしいとつけられた",
        choiceB: "ダイヤモンドの次に強度が硬いため",
        choiceC: "開発者の誕生石だから",
        choiceD: "Perlの次になれるような言語になってほしいとつけられた",
        answer: "Perlの次になれるような言語になってほしいとつけられた",
    },
    {
        id: 5,
        question: "HTTPステータスコードの説明として不正解はどれ？",
        choiceA: "404 Not Found : Webページが見つからない",
        choiceB: "500 Internal Server Error : 何らかのサーバ内で起きたエラー",
        choiceC: "200 OK : リクエストが正常に処理できた",
        choiceD: "302 Found : 恒久的に移動する",
        answer: "302 Found : 恒久的に移動する",
    },
    {
        id: 6,
        question: "MVCモデルのVは何を意味する？",
        choiceA: "Vector",
        choiceB: "View",
        choiceC: "Vercel",
        choiceD: "Visual",
        answer: "View",
    },
    {
        id: 7,
        question: "プログラミング言語のPHPの最後のPの意味はどれ？",
        choiceA: "Public",
        choiceB: "Peer",
        choiceC: "Programming",
        choiceD: "Preprocessor",
        answer: "Preprocessor",
    },
    {
        id: 8,
        question: "アメリカの有名グローバル企業の「顧客は常に正しい」はどこの会社の理念？",
        choiceA: "Google",
        choiceB: "Apple",
        choiceC: "Meta(Facebook)",
        choiceD: "Amazon",
        answer: "Amazon",
    },
    {
        id: 9,
        question: "プログラミング言語Pythonの名前の由来はどれ？",
        choiceA: "開発者がイギリスのコメディ番組モンティパイソンが好きだったため",
        choiceB: "開発者が飼っていた猫の名前がパイソンだったため",
        choiceC: "開発者が蛇のパイソン(ニシキヘビ)が好きだったため",
        choiceD: "開発者が始めて持った拳銃がコルトパイソンだったため",
        answer: "開発者が好んで見ていたイギリスのコメディ番組モンティパイソンが由来",
    },
    {
        id: 10,
        question: "iPhoneのiに込められた意味は5つあり、internet、individual、instruct、informとあと1つはどれか？",
        choiceA: "identity",
        choiceB: "interest",
        choiceC: "inspire",
        choiceD: "incredible",
        answer: "inspire",
    },
]
export default fourChoiceData;
